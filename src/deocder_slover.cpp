...
DecoderSlover::DecoderSlover(int h, int w) {
	...
	if (h == 512 && w == 512)
		net.load_param("weights/sd/AutoencoderKL-512-512-fp16-opt.param");
	else if (h == 256 && w == 256)
		net.load_param("weights/sd/AutoencoderKL-256-256-fp16-opt.param");
	else {
		generate_param(h, w);
		net.load_param(("assets/tmp-AutoencoderKL-" + std::to_string(h) + "-" + std::to_string(w) + "-fp16.param").c_str());
	}
	net.load_model("weights/sd/AutoencoderKL-fp16.bin");
}



void DecoderSlover::generate_param(int height, int width)
{
	std::string line;
...
ncnn::Mat DecoderSlover::decode(ncnn::Mat sample) {
