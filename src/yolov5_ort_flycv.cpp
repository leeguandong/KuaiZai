...
//
//
//Yolov5ORT::Yolov5ORT(const std::string modelPath, const bool isGPU = true, const fcv::Size inputSize = fcv::Size(640, 640)) {
...
//
//	//this->inputImageShape = fcv::Size2f(inputSize);
//	this->inputImageShape = fcv::Size(inputSize);
//}
//
//std::vector<Detection> Yolov5ORT::detect(fcv::Mat& image, const float confThreshold = 0.4, const float iouThreshold = 0.45) {
//	/*
...
//
//	fcv::Size resizedShape = fcv::Size((int)inputTensorShape[3], (int)inputTensorShape[2]);
...
//}
//
//void Yolov5ORT::preprocessing(fcv::Mat& image, float*& blob, std::vector<int64_t>& inputTensorShape) {
//	/*
...
//	*/
//	fcv::Mat resizedImage, floatImage; // 尺寸调整，类型转换
//	//fcv::cvtColor(image, resizedImage, fcv::COLOR_BGR2RGB);
//	fcv::cvt_color(image, resizedImage, fcv::ColorConvertType::CVT_PA_BGR2PA_RGB); // 
//
//	letterbox(resizedImage, resizedImage, this->inputImageShape, fcv::Scalar(114, 114, 114), this->isDynamicInputShape, true, 32);
//
//	// 更新输入张量的高度和宽度，将其设为调整后的 resizedImage 的高度和宽度,rows是高度，cols是宽度。
//	//inputTensorShape[2] = resizedImage.rows;
//	//inputTensorShape[3] = resizedImage.cols;
//	inputTensorShape[2] = resizedImage.height;
//	inputTensorShape[3] = resizedImage.width;
//
//	// 使用 convertTo 函数将调整后的 resizedImage 数据从 uchar 类型转换为 float 类型，并且进行归一化处理，将像素值缩放到 [0, 1] 范围内
//	//resizedImage.convertTo(floatImage, CV_32FC3, 1 / 255.0);
//	resizedImage.convert_to(floatImage, fcv::FCVImageType::PKG_RGB_F32, 1 / 255.0);
...
//	blob = new float[floatImage.width * floatImage.height * floatImage.channels()];
//
//	//cv::Size floatImageSize{ floatImage.cols,floatImage.rows };
//	fcv::Size floatImageSize{ floatImage.width,floatImage.height };
//	/*
